...
$(TARGET).la: $(LT_OBJS)
	$(LIBTOOL) --mode=link --tag=CC $(CC) -release $(RADIUSD_ORIGINAL_VERSION_STRING) \
...
install:
	if [ "x$(TARGET)" != "x" ]; then \
	    ...
	    rm -f $(R)$(libdir)/$(TARGET)-$(RADIUSD_ORIGINAL_VERSION_STRING).la; \
	    ln -s $(TARGET).la $(R)$(libdir)/$(TARGET)-$(RADIUSD_ORIGINAL_VERSION_STRING).la || exit $$?; \
	fi
