...
static int dynamic_centrale_client_authorize(UNUSED void *instance, REQUEST *request)
{
    ...
    if ((request->packet->vps != NULL) || (request->parent != NULL)) {
        radlog(L_ERR, "rlm_dynamic_centrale_clients: Improper configuration");
        return RLM_MODULE_NOOP;
    }

    if (!request->client || !request->client->cs) {
        radlog(L_ERR, "rlm_dynamic_centrale_clients: Unknown client definition");
        return RLM_MODULE_NOOP;
    }

    cp = cf_pair_find(request->client->cs, "directory");
    if (!cp) {
        radlog(L_ERR, "rlm_dynamic_centrale_clients: No directory configuration in the client");
        return RLM_MODULE_NOOP;
    }      
           
    value = cf_pair_value(cp);
    if (!value) {
        radlog(L_ERR, "rlm_dynamic_centrale_clients: No value given for the directory entry in the client.");
        return RLM_MODULE_NOOP;
    }
    ...
    if ((strlen(hostname) + strlen(value)) >= sizeof(buffer)) {
        radlog(L_ERR, "rlm_dynamic_centrale_clients: Directory name too long");
        return RLM_MODULE_NOOP;
    }

    if (!snprintf(buffer, sizeof(buffer), "%s/%s", value, hostname)) {
        radlog(L_ERR, "rlm_dynamic_centrale_clients: Unable to make cmd line");
    ...
    result = radius_exec_program(cmdline, request, TRUE, NULL, 0, 10000, NULL, NULL, FALSE);

    if (result != 0) {
        radlog(L_ERR, "rlm_dynamic_centrale_clients: External script '%s' failed", cmdline);
        return RLM_MODULE_FAIL;
    }
