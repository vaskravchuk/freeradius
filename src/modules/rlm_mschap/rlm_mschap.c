...
{
    static AUTH_SP_ATTR procs[2] = { (AUTH_SP_ATTR){MSCHAP_RESPONSE_ATTR, NT_CHALLENGE_PR, NULL, NULL},
    								 (AUTH_SP_ATTR){MSCHAP_CHALLENGE_ATTR, NT_CHALLENGE_RESPONSE_PR, NULL, NULL} };
	...
	if (!do_ntlm_auth) {
		 ...
		 */
		if (!password) {
			log_request(request, 1, "MSCHAPV2 FAILED (FAILED: No NT/LM-Password.  Cannot perform authentication)");
		...
		if (memcmp(buffer, "NT_KEY: ", 8) != 0) {
			log_request(request, 1, "MSCHAPV2 FAILED (Invalid output from ntlm_auth: expecting NT_KEY)");
		...
		if (strlen(buffer + 8) < 32) {
			log_request(request, 1, "MSCHAPV2 FAILED (Invalid output from ntlm_auth: NT_KEY has unexpected length)");
		...
		if (fr_hex2bin(buffer + 8, nthashhash, 16) != 16) {
			log_request(request, 1, "MSCHAPV2 FAILED (Invalid output from ntlm_auth: NT_KEY has non-hex values)");
...
#define inst ((rlm_mschap_t *)instance)
	 ...
	 */
	if (!response) {
		log_request(request, 1, "MSCHAPV2 FAILED (Found MS-CHAP-Challenge, but no MS-CHAP-Response)");
		RDEBUG2("Found MS-CHAP-Challenge, but no MS-CHAP-Response.");
		return RLM_MODULE_NOOP;
	}

	if (pairfind(request->config_items, PW_AUTH_TYPE)) {
		log_request(request, 1, "MSCHAPV2 FAILED (WARNING: Auth-Type already set.  Not setting to MS-CHAP)");
...
#define inst ((rlm_mschap_t *)instance)
	...
	challenge = pairfind(request->packet->vps, PW_MSCHAP_CHALLENGE);
	if (!challenge) {
		log_request(request, 1, "MSCHAPV2 FAILED (ERROR: You set 'Auth-Type = MS-CHAP' for a request that does not contain any MS-CHAP attributes)");
	...
	if (response) {
		...
		if (challenge->length < 8) {
			log_request(request, 1, "MSCHAPV2 FAILED (MS-CHAP-Challenge has the wrong format)");
		 ...
		 */
		if (response->length < 50) {
			log_request(request, 1, "MSCHAPV2 FAILED (MS-CHAP-Response has the wrong format)");
			radlog_request(L_AUTH, 0, request, "MS-CHAP-Response has the wrong format.");
			return RLM_MODULE_INVALID;
		}
		...
		if (!username) {
			log_request(request, 1, "MSCHAPV2 FAILED (We require a User-Name for MS-CHAPv2)");
		...
		log_request(request, 1, "MSCHAPV2 FAILED (You set 'Auth-Type = MS-CHAP' for a request that does not contain any MS-CHAP attributes)");
		RDEBUG("ERROR: You set 'Auth-Type = MS-CHAP' for a request that does not contain any MS-CHAP attributes!");
		return RLM_MODULE_INVALID;
	}
