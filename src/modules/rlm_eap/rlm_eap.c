...
static int eap_authenticate(void *instance, REQUEST *request)
{
	...
	if (rcode == EAP_INVALID) {
		radlog_eaphandler_portnox(handler, 1, "EAP FAILED (EAP_INVALID)");
		/*
	...
	if (rcode == EAP_INVALID || rcode == EAP_FAIL) {
		if (rcode == EAP_FAIL) {
			radlog_eaphandler_portnox(handler, 1, "EAP FAILED (EAP_FAIL)");
		}
		eap_fail(handler);
	...
	if ((request->options & RAD_REQUEST_OPTION_PROXY_EAP) != 0) {
		...
		radlog_eaphandler_portnox(handler, 1, "Not-EAP proxy set.  Not composing EAP");
	...
	if (request->proxy != NULL) {
		...
		radlog_eaphandler_portnox(handler, 1, "Tunneled session will be proxied.  Not doing EAP");
	...
	if (((handler->eap_ds->request->code == PW_EAP_REQUEST) &&
		...
		if (!eaplist_add(inst, handler)) {
			radlog_eaphandler_portnox(handler, 1, "EAP FAILED (Failed adding handler to the list)");
			eap_fail(handler);
			eap_handler_free(inst, handler);
			return RLM_MODULE_FAIL;
		}
