...
#
######################################################################
freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.gz: .git
	git archive --format=tar --prefix=freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING)/ v2.x.x | gzip > $@

freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.gz.sig: freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.gz
	gpg --default-key aland@freeradius.org -b $<

freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.bz2: .git
	git archive --format=tar --prefix=freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING)/ v2.x.x | bzip2 > $@

freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.bz2.sig: freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.bz2
	gpg --default-key aland@freeradius.org -b $<

# high-level targets
.PHONY: dist-check
dist-check: redhat/freeradius.spec suse/freeradius.spec debian/changelog
	@if [ `grep ^Version: redhat/freeradius.spec | sed 's/.*://;s/ //'` != "$(RADIUSD_ORIGINAL_VERSION_STRING)" ]; then \
		cat redhat/freeradius.spec | sed 's/^Version: .*/Version: $(RADIUSD_ORIGINAL_VERSION_STRING)/' > redhat/.foo; \
		mv redhat/.foo redhat/freeradius.spec; \
		echo redhat/freeradius.spec 'Version' needs to be updated; \
		exit 1; \
	fi
	@if [ `grep ^Version: suse/freeradius.spec | sed 's/.*://;s/ //'` != "$(RADIUSD_ORIGINAL_VERSION_STRING)" ]; then \
		cat suse/freeradius.spec | sed 's/^Version: .*/Version: $(RADIUSD_ORIGINAL_VERSION_STRING)/' > suse/.foo; \
		mv suse/.foo suse/freeradius.spec; \
		echo suse/freeradius.spec 'Version' needs to be updated; \
		exit 1; \
	fi
	@if [ `head -n 1 debian/changelog | sed 's/.*(//;s/\+.*).*//;s/-1).*//;'`  != "$(RADIUSD_ORIGINAL_VERSION_STRING)" ]; then \
		echo debian/changelog needs to be updated; \
		exit 1; \
	fi

dist: dist-check freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.gz freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.bz2

dist-sign: freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.gz.sig freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.bz2.sig

dist-publish: freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.gz.sig freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.gz freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.gz.sig freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.bz2 freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.gz.sig freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.bz2.sig
...
#  to do!
#
dist-tag: freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.gz freeradius-server-$(RADIUSD_ORIGINAL_VERSION_STRING).tar.bz2
	@echo "git tag release_`echo $(RADIUSD_ORIGINAL_VERSION_STRING) | tr .- __`"
